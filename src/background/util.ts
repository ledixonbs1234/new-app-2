...
export const convertBlobsToBlob = async (blobs: Blob[]): Promise<Blob> => {
  ...
  for (let index = 0; index < blobs.length; index++) {
    ...
    const pdfDoc = await PDFLib.PDFDocument.load(
      await pdfBlobToArrayBuffer(element),
    );
...
export const createOrActiveTab = async (
  ...
  for (const tab of tabs) {
    const isRight = isExactly
      ? tab.url === urlPrefix
      : tab.url?.includes(urlPrefix);
    if (isRight) {
      if (isReload) {
 ...
 */
export function waitForTabLoadAfterAction(
  tabId: number,
  originalUrl?: string,
    ...
    const listener = (
      ...
      chrome.tabs.get(tabId, (currentTab) => {
        if (chrome.runtime.lastError) {
          ...
          return reject(
            new Error(
              `Lỗi khi lấy chi tiết tab ${tabId}: ${chrome.runtime.lastError.message}`,
            ),
          );
        }
        const currentUrl = currentTab?.url;
        console.log(
          `Tab ${tabId} chuyển sang trạng thái 'complete' tại URL: ${currentUrl}`,
        );
        ...
        if (
          ...
          console.log(
            `Tab ${tabId} đã tải xong và điều hướng thành công tới: ${currentUrl}`,
          );
          cleanup();
          resolve(currentTab);
        } else if (currentUrl && currentUrl === originalUrl) {
          console.log(
            `Tab ${tabId} complete nhưng URL (${currentUrl}) chưa đổi so với ban đầu (${originalUrl}). Tiếp tục chờ...`,
          );
          ...
          console.log(
            `Tab ${tabId} complete nhưng vẫn ở trang login (${currentUrl}). Có thể đăng nhập thất bại.`,
          );
